import type { HostelPageParams } from './hostel.types';

/**
 * Centralized React Query key factory for the hostel feature.
 *
 * Using a factory pattern (rather than plain string arrays) gives us:
 *  - Type safety on all key parameters.
 *  - Easy partial invalidation (e.g. invalidate all hostel lists without
 *    knowing every individual key variant).
...
export const hostelKeys = {
    ...
    /** Public paginated list of active hostels. */
