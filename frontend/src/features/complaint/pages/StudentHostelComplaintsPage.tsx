...
import { AlertCircle, ArrowLeft, ChevronRight, Lock } from 'lucide-react';
...
import { useHostelComplaintsForStudent } from '../hooks/complaint.hooks';
 ...
 * upvote/downvote complaints raised by others, not just the ones they
 * personally authored, so patterns visible across many residents (e.g. a
 * recurring maintenance issue) surface through community voting rather than
 * staying siloed in each student's own complaint list.
 *
 * Access is scoped server-side to hostels the student has (or has had) a
 * genuine booking with (APPROVED/CHECKED_IN/CHECKED_OUT) — a student who has
 * never booked a hostel gets a 403, shown below as a clear explanatory state
 * rather than a generic error.
 *
 * No AppLayout — rendered inside <AppLayout> route wrapper in AppRoutes (§11).
 *
 * Route: {@code /student/hostels/:hostelId/complaints}
 */
export default function StudentHostelComplaintsPage() {
    ...
    const {
        ...
        refetch,
        isFetching,
    } = useHostelComplaintsForStudent(hostelId, {
        page: apiParams.page,
        size: apiParams.size,
    ...
    const isForbidden = (error as ApiError | undefined)?.status === 403;
        ...
        >
            ...
            <div className="space-y-3">
                ...
                <PageHeader
                    ...
                    description="Complaints raised by other students here — vote to help the manager prioritize."
                />
            </div>
            ...
            {isForbidden ? (
                <EmptyState
                    ...
                    title="Not available for this hostel"
                    description="You can only view and vote on complaints for a hostel you have (or have had) a booking with."
                    action={
                        <Button
                            variant="outline"
                            size="sm"
                        ...
                        <EmptyState
                            ...
                            description={
                                hasFilter
                                    ...
                                    : 'No complaints have been raised for this hostel yet.'
                            }
                            action={
                                hasFilter ? (
                                    <Button
